...
import {FASHION, Products, fashionANDBeauty} from '../products';
...
const Fashion = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = FASHION.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= fashionANDBeauty.length) {
