...
import {INTERIORS, Products, TRAVEL} from '../products';
...
const Interiors = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = INTERIORS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= INTERIORS.length) {
