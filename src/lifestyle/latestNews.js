...
import {ENTERTAINMENT, INTERIORS, LATESTNEWS, MENS_MOTORS, Products, TRAVEL} from '../products';
...
const LatestNews = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
    LATESTNEWS
      const newItems = LATESTNEWS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= LATESTNEWS.length) {
