...
import {HEALTH_and_BEING, Products, TRAVEL} from '../products';
...
const Healthandwellbeing = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = HEALTH_and_BEING.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= HEALTH_and_BEING.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Health and Wellbeing'}/>
