...
import {CRISTMAS, ENTERTAINMENT, INTERIORS, LATESTNEWS, MENS_MOTORS, Products, TRAVEL} from '../products';
...
const Cristmas = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
    LATESTNEWS
      const newItems = CRISTMAS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= CRISTMAS.length) {
