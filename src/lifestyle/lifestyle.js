...
import {LIFE_STYLE, Products, TRAVEL} from '../products';
...
const Lifestyle = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = LIFE_STYLE.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= LIFE_STYLE.length) {
