...
import {ENTERTAINMENT, INTERIORS, MENS_MOTORS, Products, TRAVEL} from '../products';
...
const Entertainment = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = ENTERTAINMENT.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= ENTERTAINMENT.length) {
