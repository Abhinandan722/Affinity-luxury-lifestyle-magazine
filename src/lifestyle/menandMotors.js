...
import {INTERIORS, MENS_MOTORS, Products, TRAVEL} from '../products';
...
const MenandMotors = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = MENS_MOTORS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= MENS_MOTORS.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Men and Motors'}/>
