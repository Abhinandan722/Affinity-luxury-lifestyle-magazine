...
import {travel_Review} from '../products';
...
const TravelAndReviews = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = travel_Review.slice(
        (currentPage - 1) * 5,
        currentPage * 5,
      );
      ...
      if (homescreendata.length + newItems.length >= travel_Review.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Travel And Reviews'}/>
...
export default TravelAndReviews;
