...
import {Mini_Breaks, Products, TRAVEL, travel_destinations} from '../products';
...
const MiniBreaks = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = Mini_Breaks.slice(
        (currentPage - 1) * 5,
        currentPage * 5,
      );
      ...
      if (homescreendata.length + newItems.length >= Mini_Breaks.length) {
