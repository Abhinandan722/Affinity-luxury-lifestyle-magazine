...
import {Products, TRAVEL} from '../products';
...
const LatestTravelNews = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = TRAVEL.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= TRAVEL.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Latest Travel News'}/>
