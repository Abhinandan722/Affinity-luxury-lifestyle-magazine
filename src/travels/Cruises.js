...
import {cruises, travel_Review} from '../products';
...
const Cruises = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = cruises.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= cruises.length) {
              ...
              renderItem={({item, index}) => {
                return (
                  <View>
                    ...
                    <Card item={item} navigationn={() => navigation.navigate('detialpage', {item})} />
