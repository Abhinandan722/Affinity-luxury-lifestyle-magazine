...
import {Products, TRAVEL, travel_destinations} from '../products';
...
const TravelANDdestinations = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = travel_destinations.slice(
        (currentPage - 1) * 5,
        currentPage * 5,
      );
      ...
      if (
        homescreendata.length + newItems.length >=
        travel_destinations.length
      ) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'TRAVEL AND DESTINATIONS'}/>
...
export default TravelANDdestinations;
