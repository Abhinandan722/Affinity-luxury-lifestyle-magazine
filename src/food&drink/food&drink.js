...
import {
  ...
  import {FOOD_DRINK, LIFE_STYLE, Products, TRAVEL} from '../products';
...
import Header from '../COMPONENTS/headers/header';
  ...
  const FoodANDdrink = () => {
    ...
    const loadMoreData = () => {
      ...
      setTimeout(() => {
        const newItems = FOOD_DRINK.slice((currentPage - 1) * 5, currentPage * 5);
        setHomescreendata([...homescreendata, ...newItems]);
        setCurrentPage(currentPage + 1);
  
        if (homescreendata.length + newItems.length >= FOOD_DRINK.length) {
    ...
    return (
      <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
        <View style={{flex: 1, backgroundColor: 'white'}}>
          ...
          <Header category={'Food and drink'}/>
