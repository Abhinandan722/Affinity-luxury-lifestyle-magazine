...
import {COCKTAIL, LATEST_TRAVEL_of_drink_food, LIFE_STYLE, Products, TRAVEL, restaurant_reviews, wine_spirit} from '../products';
...
const Cocktails = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = COCKTAIL.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= COCKTAIL.length) {
