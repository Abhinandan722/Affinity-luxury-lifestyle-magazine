...
import {LIFE_STYLE, Products, RACIPES_TIPS, TRAVEL} from '../products';
...
const RacipesANDTips = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = RACIPES_TIPS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= RACIPES_TIPS.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Recipes  and Tips'}/>
