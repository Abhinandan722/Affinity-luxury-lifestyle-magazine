...
import {LATEST_TRAVEL_of_drink_food, LIFE_STYLE, Products, TRAVEL, restaurant_reviews, wine_spirit} from '../products';
...
const WineANDspirit = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = wine_spirit.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= wine_spirit.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Wine and Spirit'}/>
