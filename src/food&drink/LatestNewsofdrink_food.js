...
import {LATEST_TRAVEL_of_drink_food, LIFE_STYLE, Products, TRAVEL} from '../products';
...
const LatestNewsofdrink_food = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = LATEST_TRAVEL_of_drink_food.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= LATEST_TRAVEL_of_drink_food.length) {
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow: 1}}>
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Header category={'Food and Drink Latest News'}/>
...
export default LatestNewsofdrink_food;
