...
import {COMPETITIONS, LATEST_TRAVEL_of_drink_food, LIFE_STYLE, Products, TRAVEL, restaurant_reviews, wine_spirit} from '../products';
...
const Competitions = () => {
  ...
  const loadMoreData = () => {
    ...
    setTimeout(() => {
      const newItems = COMPETITIONS.slice((currentPage - 1) * 5, currentPage * 5);
      setHomescreendata([...homescreendata, ...newItems]);
      setCurrentPage(currentPage + 1);

      if (homescreendata.length + newItems.length >= COMPETITIONS.length) {
